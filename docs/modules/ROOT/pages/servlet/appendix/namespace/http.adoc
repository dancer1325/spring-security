...
* **request-matcher**
* == `RequestMatcher` strategy / used | `FilterChainProxy` + beans to match incoming requests / -- created by -- `intercept-url`
    ** available options
        *** `mvc` -- for Spring MVC --
        *** `ant` -- for ant --
        *** `regex` -- for regular-expression --
        *** `ciRegex` -- for case-insensitive regular-expression --
* TODO:
