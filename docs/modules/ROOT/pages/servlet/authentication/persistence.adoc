...
* Check javadoc:org.springframework.security.web.context.SecurityContextHolderFilter[]
* allows
    ** request1's `SecurityContext` -- can be loaded, via the xref::servlet/authentication/persistence.adoc#securitycontextrepository[`SecurityContextRepository`], into -- request2's `SecurityContext`
        *** steps
            **** image:{icondir}/number_1.png[]  Before running the rest of the application, `SecurityContextHolderFilter` -- loads the `SecurityContext`, from the `SecurityContextRepository`, into -- `SecurityContextHolder`
            **** image:{icondir}/number_2.png[] application is ran

image::{figures}/securitycontextholderfilter.png[]

* vs xref:servlet/authentication/persistence.adoc#securitycontextpersistencefilter[`SecurityContextPersistenceFilter`]
    ** ⚠️ `SecurityContextHolderFilter` ONLY loads (NOT save) the `SecurityContext` ⚠️
        *** -> you need explicitly to save the `SecurityContext`
